...
class LinformerAttention(tf.keras.layers.Layer):
    ...
    def __init__(
        self, hidden_size, num_heads, proj_dim=None, dropout=0.1, name="attention"
    ):
        ...
        if proj_dim is not None:
            self.e = tf.keras.layers.Dense(proj_dim, name="e_proj")
            self.f = tf.keras.layers.Dense(proj_dim, name="f_proj")
    ...
    def call(self, batch, attention_mask=None, training=False):
        ...
        v_out, k_out = self.v(batch), self.k(batch)  # bs, seq, nfeats
        if self.proj_dim is not None:
            v_out, k_out = self.proj_sequence(k_out, v_out)
        k_out = self._prepare_either_qkv(k_out, head_size)
        v_out = self._prepare_either_qkv(v_out, head_size)
    ...
    def proj_sequence(self, k_out, v_out):
        k_out, v_out = tf.transpose(k_out, perm=[0, 2, 1]), tf.transpose(
            v_out, perm=[0, 2, 1]
        )
        k_out, v_out = self.e(k_out), self.f(v_out)
        return tf.transpose(k_out, perm=[0, 2, 1]), tf.transpose(v_out, perm=[0, 2, 1])
...
class LinformerLayer(tf.keras.layers.Layer):
    def __init__(
        self,
        hidden_size,
        num_heads,
        ...
        self.attention = LinformerAttention(
            hidden_size, num_heads, proj_dim=proj_dim, dropout=dropout, name="attention"
        )
        self.dropout = tf.keras.layers.Dropout(dropout)
...
class Wav2Vec2Lincoder(tf.keras.layers.Layer):
    def __init__(
        self,
        hidden_size,
        num_heads,
